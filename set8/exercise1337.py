...
def list_please() -> list:
    ...
    return [1, 'apple', True, 3.14, 'banana']
...
def take_five(some_number) -> int:
    ...
    return some_number - 5
result = take_five(10)
print(result)  # Output: 5 (10 - 5 = 5)

result = take_five(7)
print(result)  # Output: 2 (7 - 5 = 2)
...
greeting = greet("Li")
print(greeting)  # Output: "Well hello, Li"

greeting = greet()
print(greeting)  # Output: "Well hello, Towering Timmy" (using the default name)
...
def one_counter(input_list=[1, 4, 1, 5, 1, 1]) -> int:
    ...
    return count
count = one_counter([1, 2, 1, 3, 4, 1])
print(count)  # Output: 3 (there are three occurrences of 1 in the list)

count = one_counter([0, 0, 0, 0])
print(count)  # Output: 0 (there are no occurrences of 1 in the list)
...
def n_counter(search_for_this, input_list=[1, 4, 1, 5, 1, 1]) -> int:
    ...
    return count
count = n_counter(1, [1, 2, 1, 3, 4, 1])
print(count)  # Output: 3 (there are three occurrences of 1 in the list)

count = n_counter(5, [0, 0, 0, 0])
print(count)  # Output: 0 (there are no occurrences of 5 in the list)
...
def set_it_on_fire(input_string="very naughty boy") -> str:
    ...
    interleaved_string = fire_emoji.join(uppercase_string)

    # Add 🔥 at both ends of the string
    final_string = fire_emoji + interleaved_string + fire_emoji

    return final_string
result = set_it_on_fire("very naughty boy")
print(result)  # Output: "🔥V🔥E🔥R🔥Y🔥 🔥N🔥A🔥U🔥G🔥H🔥T🔥Y🔥 🔥B🔥O🔥Y🔥"
...
def pet_filter(letter="a") -> List:
    ...
    # fmt: on
    filtered = [pet for pet in pets if letter in pet.lower()]
...
def make_filler_text_dictionary() -> Dict:
    ...
    for length in range(3, 8):
        ...
        for _ in range(4):  # Get 4 words for each word length
            ...
            word_list.append(response.text.strip())  # Remove leading/trailing whitespaces
        filler_text_dictionary[length] = word_list

    return filler_text_dictionary

# Example usage:
result = make_filler_text_dictionary()
print(result)
...
def random_filler_text(number_of_words=200) -> str:
    ...
    word_lengths = list(my_dict.keys())  # Available word lengths
...
def fast_filler(number_of_words=200) -> str:
    ...
    if os.path.isfile(fname):
        with open(fname, "r") as inFile:
            my_book = json.load(inFile)
    else:
        my_book = make_filler_text_dictionary()
        with open(fname, "w") as outFile:
            json.dump(my_book, outFile)
    words = []
    ...
    for _ in range(number_of_words):
        ...
        try:
            words.append(my_book[word_length][word_index])
        except KeyError:
            words.append(my_book[str(word_length)][word_index])
